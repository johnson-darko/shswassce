...
export async function initializeDatabase() {
  ...
  try {
    ...
    const universityCount = await db.select({ count: count() }).from(universities);
    const hasData = universityCount[0].count > 0;

    if (hasData) {
      console.log(`Database already initialized with ${universityCount[0].count} universities`);
      return;
    }
    ...
    // Sample Requirements - Enhanced with admission tracks
    ...
    // Don't throw error, let app continue with empty database
  }
}