...
async function checkEligibility(grades: WassceeGrades): Promise<EligibilityResult[]> {
  ...
  for (const university of universities) {
    ...
    for (const program of programs) {
      ...
      const result = checkEnhancedEligibility(grades, university, program, requirement);
      results.push(result);
    }
  }
  
  // Sort results: eligible/multiple_tracks first, then borderline, then not eligible
  results.sort((a, b) => {
    const statusOrder = { 'eligible': 0, 'multiple_tracks': 0, 'borderline': 1, 'not_eligible': 2 };
